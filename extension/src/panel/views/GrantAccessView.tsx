  ...
  return (
    <div className="scroll-area flex-1 px-3 py-4">
      ...
      <Meta className="mt-1.5 max-w-[38ch]">
        Orbit can&apos;t see a page until you allow it. Turn on the sites you
        want it to work on — you can switch them off again at any time.
      </Meta>
        ...
        <ul className="space-y-1">
          {KNOWN_SITES.filter((s) => s.label !== "Twitter").map((s) => {
            ...
            return (
              <li
                key={s.origin}
                className="flex items-center gap-2 border-t border-[var(--border)] py-1.5 text-[13px] first:border-t-0"
              >
                <span className="flex-1">{s.label}</span>
                {on ? (
                  <>
                    <span className="inline-flex items-center gap-1 text-[11px] text-[var(--primary)]">
                      <Check size={11} />
                      on
                    </span>
                    <button
                      onClick={() =>
                        void revokeSites([s.origin]).then(refresh)
                      }
      ...
      <Meta className="mt-4 max-w-[38ch]">
        Orbit only reads the page you&apos;re looking at, only while the panel is
        open, and never sends the page text anywhere except your own Orbit
        account.
      </Meta>
    </div>
  );
