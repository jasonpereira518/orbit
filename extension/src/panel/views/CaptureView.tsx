...
import { Button, MicroLabel, Meta, Section } from "../components/ui";
...
export function CaptureView({
  ...
  const save = async (force = false) => {
    ...
    try {
      ...
      // A duplicate is a discovery, not a failure — surface the match rather
      // than an error the user can only apologise to.
      setError(
        apiError.code === "duplicate"
          ? "Orbit already has someone like this."
          : apiError.message
      );
    } finally {
      setBusy(false);
    ...
    <>
      <div className="scroll-area flex-1">
        ...
        {state.starters.length > 0 || state.startersLoading ? (
          <Section>
            <StarterList
              title="What you could open with"
      ...
      {error ? (
        <div className="border-t border-[var(--border)] bg-[var(--accent)] px-3 py-2">
          <Meta className="text-[var(--foreground)]">{error}</Meta>
          {error.startsWith("Orbit already has") ? (
            <button
              onClick={() => void save(true)}
              className="mt-1 text-[11px] text-[var(--primary)] underline-offset-2 hover:underline"
            >
              Add anyway
            </button>
          ) : null}
