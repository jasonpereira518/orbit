...
export type PanelState = {
  ...
  /** Why the read failed, discriminated — so the panel can branch on the
   *  actual cause instead of pattern-matching the display copy. */
...
export function usePanel() {
  ...
  // The last page Orbit actually resolved, kept independent of `state` so a
  // failed re-resolve of that *same* page can fall back to it (the "Offline —
  // showing what Orbit had" promise) without ever showing it under a
  // *different* page's identity zone.
    ...
    async (page: PageContext, contactId: string | null) => {
      ...
      try {
        ...
        setState((s) => ({
          ...
          startersDegradedReason: result.degradedReason ?? null,
          startersLoading: false,
        }));
      } catch {
  ...
  const run = useCallback(async () => {
    ...
    // `resolved` is deliberately NOT cleared here — see below, once we know
    // whether this run is for the same page as last time.
    setState((s) => ({
      ...s,
      resolving: true,
      starters: [],
    ...
    // Same page as the last successful resolve → keep whatever `resolved` is
    // already in state (so a failed re-resolve still has it to fall back on).
    // A genuinely different page → clear it, so a resolve failure there can't
    // surface a *previous* person's record under the new identity zone.
    const samePage = lastResolvedRef.current?.url === page.url;
    setState((s) => ({ ...s, page, resolved: samePage ? s.resolved : null }));
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      lastResolvedRef.current = { url: state.page.url, resolved };
