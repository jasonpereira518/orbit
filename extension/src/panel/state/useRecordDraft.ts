...
const CORE: FieldKey[] = ["fullName"];
// Order matters here — it's the order rows render in. Title/Company used to be
// pinned as CORE and always rendered, even empty (a bare "—"), which is
// exactly the "form with holes" `Section` itself warns against. They now
// collapse into the same reveal-on-value-or-click chip group as the other
// optional fields.
const REVEALABLE: FieldKey[] = ["title", "company", "location", "email", "school"];
...
export function useRecordDraft(
  ...
  const available = REVEALABLE.filter((key) => !visible.includes(key)).map(
  ...
  return {
    ...
    toFields: () => ({
      ...
      // Prefer the local DOM read, same precedence as every other field —
      // `suggested` only backs it up if the adapter found nothing.
      photoUrl: page?.identity.photoUrl?.value || suggested?.photoUrl || undefined,
