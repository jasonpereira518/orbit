/**
 * PORTED from the Orbit app's src/lib/company-brand.ts.
 *
 * The extension is a separate bundle and cannot import from the Next app, so
 * this is an intentional copy. Keep it in step when brands are added there —
 * a company showing one colour in the app and another in the panel is exactly
 * the kind of small wrongness that makes an extension feel bolted on.
 *
 * normalizeCompanyName is inlined from the app's src/lib/company-name.ts.
 */
