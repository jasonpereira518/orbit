...
const byN = (label) => new Map(merged[label].filter((r) => !r.failed).map((r) => [r.n, r]));
const B = byN(before);
const A = byN(after);
...
const reps = sizes.map((n) => `${B.get(n).reps}/${A.get(n).reps}`)[0];
...
console.log(st.join("\n"));
console.log(`\nMedians of ${reps} repetitions per build and size. Control FPS on an empty page (${before} / ${after}): ${ctl}.`);
