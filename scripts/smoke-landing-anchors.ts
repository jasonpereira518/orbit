 ...
 *    existed, so every heading landed under the header. The rule is scoped to .landing-root,
 *    because the dashboard's reminders card also uses id="reminders" and a bare id selector
 *    outranks its own margin class.
...
function main() {
  ...
  for (const { id } of LANDING_SECTIONS) {
    const rule = marginRules.find((r) => r.selectors.includes(`.landing-root #${id}`));
    const rem = rule ? /scroll-margin-top\s*:\s*([\d.]+)rem/.exec(rule.body)?.[1] : undefined;
    check(`.landing-root #${id} has a scroll-margin-top`, Boolean(rule));
    check(
      `.landing-root #${id}'s margin equals LANDING_HEADER_SCROLL_OFFSET (${LANDING_HEADER_SCROLL_OFFSET}px)`,
