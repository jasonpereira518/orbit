...
function fakeParse(corpus: string): CaptureParseResult {
  const card = (key: string, parsed: Parsed) => ({ key, notes: NOTE, parsed, duplicates: [], suggestedMergeId: null, sharedNoteTexts: [], interactionDate: "2026-09-14", interactionType: "meeting_note", opportunities: [], impliedSteps: [], cadence: null });
  return {
    items: [
    ...
    suggestedReminders: [
      { key: "0-next week", title: "Follow up with Priya about the PM role", description: null, rawDatePhrase: "next week", dueDateIso: "2026-09-21", yearInferred: false, personName: "Priya Raman", actionKind: "follow_up", confidenceScore: 80, sourceExcerpt: "Follow up with Priya next week about the PM role.", dateBasis: "relative", anchorIso: "2026-09-15", origin: "explicit", rationale: null },
    ],
...
/**
 * The smoke runner shares one PGlite across every script, and several (admin analytics'
 * engagement counts) total saved captures across ALL users — so this leaves nothing behind.
 */
