  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <p className="text-sm font-medium text-[#3d7a6c]">Your network</p>
        <h1 className="font-[family-name:var(--font-display)] text-4xl tracking-tight text-[#0f3d3e]">
          Stay in orbit
        </h1>
              ...
              data.suggestions.map((s) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-medium text-[#0f3d3e]">{s.title}</p>
          ...
          <CardContent className="space-y-3">
            {data.reminders.length === 0 ? (
              ...
              data.reminders.map((r) => (
                <div
                  key={r.id}
                  className="flex items-start justify-between gap-3 rounded-xl border border-border/60 bg-white p-4"
                >
                  <div>
            ...
            {data.dueFollowUps.length === 0 ? (
              ...
              data.dueFollowUps.map((c) => (
                ...
                >
                  ...
                  <Badge variant="secondary">Score {c.relationshipScore}</Badge>
                </Link>
              ))
            )}
          </CardContent>
...
function StatCard({
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-white/80 p-4 backdrop-blur">
      <div className="flex items-center justify-between text-muted-foreground">
        <span className="text-xs font-medium uppercase tracking-wide">{label}</span>
        {icon}
      </div>
      <p className="mt-3 font-[family-name:var(--font-display)] text-3xl text-[#0f3d3e]">
        {value}
      </p>
    </div>
  );
