...
export const POST = extensionRoute<StartersRequest, StartersResponse>({
  ...
  handler: async ({ userId, input }) => {
    const [ctx, writingInstructions] = await Promise.all([
      buildStarterContext(userId, input.page, input.contactId),
      // From the signed-in user's own row, never from the request: the extension client has
      // no field for it, so a page cannot supply one. The starters cache keys on the whole
      // prompt, so a change to the notes refreshes the starters on its own.
