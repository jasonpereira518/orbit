...
/**
 * Re-exported so server callers have one import for the whole feature. The definitions
 * live in `feedback-report.ts` because the FORM needs them too, and this module reaches
 * `@/db` — a client component that imports anything reaching `@/db` fails the build with a
 * `node:fs` chunking error naming neither file. The dependency only points this way.
 */
export {
  FEEDBACK_AREAS,
  FEEDBACK_CATEGORIES,
