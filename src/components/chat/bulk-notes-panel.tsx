      ...
      {step === "review" && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-medium text-[#0f3d3e]">
                Review {items.length}{" "}
                {items.length === 1 ? "person" : "people"}
              </h2>
              <p className="text-sm text-muted-foreground">
                Confirm creates and merges before anything is saved.
              </p>
            </div>
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {item.suggestedMergeId && (
          <Badge variant="secondary">Likely existing contact</Badge>
        )}
      </div>
      ...
      {item.parsed.summary && (
        <p className="text-sm text-muted-foreground">{item.parsed.summary}</p>
      )}
        ...
        {item.duplicates.map((d) => (
          <label key={d.id} className="flex items-center gap-2 text-sm">
            ...
            <span>
              ...
              <Link
                href={`/contacts/${d.id}`}
                className="text-[#0f3d3e] underline"
