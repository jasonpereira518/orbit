...
/**
 * Live data for `/admin/health` — everything on the page except "Known bug signatures"
 * (excluded because it changes on a data-quality timescale, and one of its queries is an
 * unindexed scan not worth paying on every poll). Polls `/api/admin/health/live`; see
 * `use-live-poll.ts` and `presence.tsx` for why polling and not `router.refresh()`/SSE.
 *
 * Dates in `HealthLiveData` arrive as ISO strings after the JSON round trip, not `Date`
 * instances — every place below that shows one uses `RelativeTime`, which already accepts
 * `Date | string` for exactly this reason (see `relative-time.tsx`).
 */
...
export type HealthLiveData = {
  ...
  outreach: OutreachQueueHealth | null;
  ops: OpsStatus | null;
};

const HealthLiveContext = createContext<HealthLiveData | null>(null);

export function HealthLiveProvider({
  initial,
  children,
}: {
  initial: HealthLiveData;
  children: React.ReactNode;
}) {
  const data = useLivePoll<HealthLiveData>(
    "/api/admin/health/live",
    initial,
    POLL_INTERVAL_MS
  );
  return (
    <HealthLiveContext.Provider value={data}>
      {children}
    </HealthLiveContext.Provider>
  );
}

function useHealthLive(): HealthLiveData {
  const value = useContext(HealthLiveContext);
  if (!value) throw new Error("useHealthLive used outside HealthLiveProvider");
  return value;
}

const inspector = (userId: string) => `/admin/users/${encodeURIComponent(userId)}`;

function who(userId: string, email: string | null) {
  return (
...
/** "3 min ago" for the strip, which takes a string rather than a node. Accepts a
 *  server `Date` or the ISO string it becomes after a JSON round trip. */
