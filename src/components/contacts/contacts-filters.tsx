...
function SearchPill({
  ...
  const companyControl = (
    <Popover open={companyOpen} onOpenChange={onCompanyOpenChange}>
      <PopoverTrigger
        type="button"
        className={cn(
          "inline-flex h-9 shrink-0 items-center gap-1 rounded-full px-2.5 text-xs font-medium transition-colors sm:h-8 sm:px-2.5",
  ...
  const closenessControl = (
    <DropdownMenu>
      <DropdownMenuTrigger
        type="button"
        className={cn(
          "inline-flex h-9 shrink-0 items-center gap-1 rounded-full px-2.5 text-xs font-medium transition-colors sm:h-8",
          "outline-none focus-visible:ring-2 focus-visible:ring-ring/50",
          closenessActive
            ? "bg-primary/10 text-primary"
            : "text-muted-foreground hover:bg-muted hover:text-foreground"
