...
- **Instant, but never old.** A prefetched or router-cached page can be up to a minute old when it is shown (`staleTimes`; Next will not keep a full prefetch for less than 30 s). Every app page renders `<RenderStamp />` (`src/components/layout/render-stamp.tsx`); if the copy on screen was rendered more than 2 s before it arrived, `FreshOnArrival` calls `router.refresh()` and the fresh page replaces it in place, with no skeleton. A new page must render one. A refresh re-prefetches every visible link, so don't prefetch in full at page load what a hover can prefetch just before the click (the desktop sidebar is intent-only for this reason; the phone nav keeps its eager prefetch because a tap has no hover). Chat history does the same for threads read on hover (`src/lib/chat-thread-prefetch.ts`).
...
- **Every foreign key gets an index that LEADS with the FK column.** Postgres enforces ON DELETE CASCADE / SET NULL with `WHERE fk_col = $1` per parent row, never adding `user_id`, so a `(user_id, contact_id)` index does not serve it and a contact delete seq-scans the child across every tenant. New child tables add a `(fk_col)` index (partial `WHERE fk_col IS NOT NULL` when mostly null) in `SCALE_DDL`.
- **No per-item statements in a loop over user data.** Batch reads with `inArray` or a `row_number() OVER (PARTITION BY …)` window, writes with one multi-row insert or `UPDATE … FROM (VALUES …)` (pattern: `persistClosenessScores`), and put a delete-then-insert in one `runAtomicWrite`. Examples: `deleteContactsForUser`, `syncMemoryChunksMany`, `recordDuplicateSuggestions`, `enqueueWebhookEvents`.
- **Resolve the account once per request.** `requireAuthenticatedUser()` returns the `user_settings` row the auth gate read; pass it on (`entitlementsFromSettings`, `AiAccess.open(userId, { row })`) and pass the resulting `AiAccess` to `completeJson`/`createEmbedding`/`streamText`. Grants — and the managed-allowance check — are still minted per model call.
- **Lists a person scrolls are capped in SQL.** Filter, dedupe and `LIMIT` in the query with `count(*) over ()` for the total, rather than loading every pending row to `.slice()` and `.length` it; histories page by keyset (`(updated_at, id)` or `id > $after`), never an unordered OFFSET.
